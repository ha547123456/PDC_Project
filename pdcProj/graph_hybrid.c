...
void load_graph(const char* graph_path, const char* partition_path) {
    ...
    // Broadcast graph data in parallel chunks
...
void process_deletions() {
    ...
    // Broadcast deleted edges and affected partitions in parallel
    #pragma omp parallel for schedule(static)
    for (int u = 1; u <= num_vertices; u++) {
...
void update_partition(int partition) {
    ...
    do {
        ...
        #pragma omp parallel for reduction(||:changed) reduction(+:updates) schedule(dynamic, 100)
...
void save_results(const char* output_path) {
    ...
    #pragma omp parallel for reduction(+:remaining_edges) schedule(dynamic, 100)
    for (int u = 1; u <= num_vertices; u++) {
...
int main(int argc, char** argv) {
    ...
    if (my_rank == 0) {
        printf("===== Parallel Graph Processing (MPI+OpenMP) =====\n");
        printf("Running with %d MPI processes\n", num_procs);
        #pragma omp parallel
        {
            #pragma omp master
            printf("Using %d OpenMP threads per process\n", omp_get_num_threads());
        }
    }
